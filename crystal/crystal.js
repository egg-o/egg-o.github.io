...
var ground = Bodies.rectangle(
    (window.innerWidth / 2) + 160, window.innerHeight - 5, window.innerWidth + 320, 160,{render: { fillStyle: '#9f86c0ff'}, isStatic: true });
...
var allbodies = [];
var bounds = Bounds.create(0, window.innerHeight, 0, window.innerHeight);

function generate() {
  console.log(allbodies.length);
  var present = Query.region(allbodies, bounds, true);
  if  (allbodies.length > 70 ) {
    return;
  }
//generating bodies
var poly;
...
for (let i = 0; i < 6; i++) {
    poly = Bodies.polygon(500 + (10 * i), 50, 5, 40, {restitution : 0.8, render: { fillStyle: fills[r]}});
    bodies[i] = poly;
}
